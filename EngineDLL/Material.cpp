...
void Material::Bind(const char* texName, unsigned int texture, int a)
{
	glUseProgram(programId);

	// Bind our texture in Texture Unit a
	glActiveTexture(GL_TEXTURE0 + a);
	glBindTexture(GL_TEXTURE_2D, texture);

	unsigned int textureId = glGetUniformLocation(programId, texName);

	// Set our "myTextureSampler" sampler to use Texture Unit a
	glUniform1i(textureId, a);
...
unsigned int Material::ReturnProgramID()
{
	return programId;
}